  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      if (url) {
        ...
        // The host [data-filled] attr can be stripped by a framework's host
        // reconciliation (e.g. the DC runtime), which would leave the
        // :host([data-filled]) .ring{display:none} rule unmatched and the
        // dashed empty-state ring drawn over a filled image. Shadow DOM is
        // not reconciled, so set the ring's display here directly.
        this._ring.style.display = 'none';
        // Drop the drop-zone grey once filled so cutout PNGs sit on whatever
        // is behind them (set here, not via CSS, since the DC runtime can
        // strip the [data-filled] host attr the selector would rely on).
        ...
        this._frame.style.background = 'rgba(0,0,0,.04)';
        this._ring.style.display = mask ? 'none' : '';
      }
    }
  }
